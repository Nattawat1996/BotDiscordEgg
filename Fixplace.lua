...
local function GroundAt(pos : Vector3)
    ...
    return pos + Vector3.new(0, 6, 0)
end

local function ensureNear(position : Vector3, maxDist : number?)
...
--== 1) สร้างแคตาล็อกแปลงทั้งหมดจากเกาะ (Land/Water) ด้วยคีย์กริดตรงเป๊ะ
local Plots = { list = {}, byKey = {} }  -- rec = { key, pos, area, part }
local function BuildPlots()
    table.clear(Plots.list); table.clear(Plots.byKey)

    for _, p in ipairs(Island:GetDescendants()) do
        if p:IsA("BasePart") then
            local n = p.Name
            -- ชื่อเริ่มด้วย Farm_split_... (บก) หรือ WaterFarm_split_... (น้ำ)
            if n:find("^Farm_split_") or n:find("^WaterFarm_split_") then
                local ic = p:GetAttribute("IslandCoord")
                if ic then
                    local key  = _k(ic.X, ic.Z)
                    local area = n:find("^WaterFarm_") and "Water" or "Land"
                    local rec  = { key = key, pos = p.Position, area = area, part = p }
                    Plots.byKey[key] = rec
                    table.insert(Plots.list, rec)
                end
            end
        end
    end

    -- มีลำดับคงที่เพื่อเดินจากบนลงล่าง/ซ้ายไปขวา (ปรับได้ตามชอบ)
    table.sort(Plots.list, function(a, b)
...
--== Snap จุดวางให้กึ่งกลางแปลงเสมอ
local function SnapPosForPlot(rec)
    -- rec: { key, pos, area, part }
    local p = rec.pos
    -- ปกติ Position ก็กลางชิ้นอยู่แล้ว; ปรับให้เป็นเลขกลม ๆ กัน drift
...
-- ใช้คีย์ X,Z → หา area จาก Plots.byKey
local function _areaByXZ(x, z)
    local rec = Plots.byKey[_k(x, z)]
    return (rec and rec.area) or "Any"
end
-- ระบุพื้นที่ของ "สัตว์" จาก DI ใน Data.Pets ก่อน ถ้าไม่มีค่อย fallback มาที่ OwnedPets[uid].GridCoord
local function petArea(uid : string)
    if not uid or uid == "" then return "Any" end

    -- 1) ลองอ่านจาก Data.Pets (ของจริงที่เซิร์ฟยืนยันแล้ว)
    local petNode = OwnedPetData:FindFirstChild(uid)
    if petNode then
        local di = petNode:FindFirstChild("DI")
        if di then
            local x = tonumber(di:GetAttribute("X") or 0) or 0
            local z = tonumber(di:GetAttribute("Z") or 0) or 0
            return _areaByXZ(x, z)
        end
    end

    -- 2) fallback: ใช้ GridCoord ที่แคชไว้ใน OwnedPets (ถ้ามี)
    local P = OwnedPets[uid]
    local gc = P and P.GridCoord
    if gc then
        return _areaByXZ(tonumber(gc.X) or 0, tonumber(gc.Z) or 0)
    end
    return "Any"
end

-- ระบุพื้นที่ของ "ไข่" จาก DI ใน Data.Egg เท่านั้น (เฉพาะไข่ที่ถูกวางแล้วถึงมี DI)
...
--== 2) ทำชุด occupied จาก “ข้อมูลจริง” ใน Data (DI) ของไข่และสัตว์
local function BuildOccupiedSetExact()
    local occ = {}

    -- ไข่ที่ “ถูกวางแล้ว” จะมี DI ใน Data.Egg
    for _, egg in ipairs(OwnedEggData:GetChildren()) do
        local di = egg:FindFirstChild("DI")
        if di then
            local key = _k(di:GetAttribute("X") or 0, di:GetAttribute("Z") or 0)
            if Plots.byKey[key] then occ[key] = true end
        end
    end

    -- สัตว์ที่ “ถูกวางแล้ว” จะมี DI ใน Data.Pets
    for _, petCfg in ipairs(OwnedPetData:GetChildren()) do
        local di = petCfg:FindFirstChild("DI")
        if di then
            local key = _k(di:GetAttribute("X") or 0, di:GetAttribute("Z") or 0)
            if Plots.byKey[key] then occ[key] = true end
        end
...
--== 3) ดึงแปลงว่างตัวแรกตามพื้นที่ที่ต้องการ
local function NextFreePlot(areaWant : string, occ : table)
    for _, rec in ipairs(Plots.list) do
        if (areaWant == "Any" or rec.area == areaWant) and not occ[rec.key] then
            return rec -- {key, pos, area}
        end
    end
    return nil
end

--== 4) วางไข่ 1 ฟองลงแปลงว่าง + อัปเดต occ (รีเฟรชจาก Data หลังวาง)
local function PlaceOneEggExact(eggInst : Instance, areaWant : string, occ : table)
    local plot = NextFreePlot(areaWant, occ)
    if not plot then return false, "no-free-plot", occ end

    local dst = GroundAt(SnapPosForPlot(plot))
    ensureNear(dst, 12)

    CharacterRE:FireServer("Focus", eggInst.Name)
    task.wait(0.45)
    CharacterRE:FireServer("Place", { DST = Vector3.new(dst.X, dst.Y, dst.Z), ID = eggInst.Name })
    task.wait(0.2)
    CharacterRE:FireServer("Focus")
    ...
    while tick() - t0 < 3 do
        local di = eggInst:FindFirstChild("DI")
        if di then ok = true break end
        task.wait(0.1)
    end

    if ok then
        -- mark ช่องนี้ และรีบิลด์ occ จากข้อมูลจริงกัน drift
        occ[plot.key] = true
        occ = BuildOccupiedSetExact()
    end
    return ok, nil, occ
end


--== 5) เลือกไข่จากกระเป๋าตามตัวกรอง (Types/Mutations)
local function PickOwnedEggByConfig()
    local typeOn = next(Configuration.Egg.Types) ~= nil
    local mutOn  = next(Configuration.Egg.Mutations) ~= nil

    for _, egg in ipairs(OwnedEggData:GetChildren()) do
        -- เฉพาะที่ “ยังไม่ถูกวาง” (ไม่มี DI)
        if egg and not egg:FindFirstChild("DI") then
            local t = egg:GetAttribute("T") or "BasicEgg"
            local m = egg:GetAttribute("M") or "None"
...
local function incomeOf(uid : string)
    local ok, val = pcall(function()
        return GetInventoryIncomePerSecByUID and GetInventoryIncomePerSecByUID(uid) or 0
    end)
    return tonumber(ok and val or 0) or 0
end

--== เลือกสัตว์จากคลังตามตัวกรอง (All / Match / Range)
local function PickPetByConfig()
    ...
    for _, petCfg in ipairs(OwnedPetData:GetChildren()) do
        ...
        -- “ยังไม่ถูกวาง” = ไม่มี DI ใน Data.Pets (ไม่ต้องสน OwnedPets)
        if not petCfg:FindFirstChild("DI") then
            local pass = false
    ...
    -- เลือกตัวที่รายได้/วิ สูงสุดก่อน
...
--== รอคอนเฟิร์มการวางด้วย DI ใน Data.Pets
local function waitPetPlacedDI(uid : string, timeout : number?)
    local petNode = OwnedPetData:FindFirstChild(uid)
    if not petNode then return false end
    local t0 = tick()
    while tick() - t0 < (timeout or 3) do
        if petNode:FindFirstChild("DI") then return true end
        task.wait(0.1)
    end
    return false
end

--== วางสัตว์ 1 ตัวลงแปลงว่าง + อัปเดต occ เมื่อยืนยันสำเร็จ (พร้อมรีเฟรช)
local function PlaceOnePetExact(petCfg : Instance, areaWant : string, occ : table)
    local plot = NextFreePlot(areaWant, occ)
    if not plot then return false, "no-free-plot", occ end
    ...
    task.wait(0.45)
    CharacterRE:FireServer("Place", { DST = Vector3.new(dst.X, dst.Y, dst.Z), ID = petCfg.Name })
    task.wait(0.2)
    CharacterRE:FireServer("Focus")

    local ok = waitPetPlacedDI(petCfg.Name, 3)
    if ok then
        occ[plot.key] = true
        occ = BuildOccupiedSetExact()
    end
...
--== วางไข่ต่อเนื่องจนกว่าจะหมดช่อง/หมดไข่
local function PlaceEggsUntilFull(areaWant : string)
    local occ = BuildOccupiedSetExact()
    local placed = 0
    while true do
        local egg = PickOwnedEggByConfig()
        if not egg then break end
        local nextPlot = NextFreePlot(areaWant, occ)
        if not nextPlot then break end
        local ok; ok, _, occ = PlaceOneEggExact(egg, areaWant, occ)
...
--== วางสัตว์ต่อเนื่องจนกว่าจะหมดช่อง/หมดตัวที่ตรงเงื่อนไข
local function PlacePetsUntilFull(areaWant : string)
    local occ = BuildOccupiedSetExact()
    local placed = 0
    while true do
        local petCfg = PickPetByConfig()
        if not petCfg then break end
        local nextPlot = NextFreePlot(areaWant, occ)
        if not nextPlot then break end
        local ok; ok, _, occ = PlaceOnePetExact(petCfg, areaWant, occ)
        if not ok then break end
        placed += 1
        task.wait(0.2)
    end
...
-- ===== Auto Collect coin from pets (FIXED) =====
task.defer(function()
    while RunningEnvirontments do
        if not Configuration.Main.AutoCollect then
            -- idle: ต้องยอมแพ้เฟรมถ้าไม่ทำงาน ไม่งั้น busy loop จะค้าง
            task.wait(0.5)
        else
            local claimed = 0
            for _, pet in pairs(OwnedPets) do
                ...
                if RE then
                    RE:FireServer("Claim")
                    claimed += 1
                    -- คืนเฟรมทุก ๆ 10 ตัว เพื่อลดกระตุกเวลาสัตว์เยอะ
                    if claimed % 10 == 0 then task.wait() end
                end
            end
            task.wait(tonumber(Configuration.Main.Collect_Delay) or 3)
        end
    end
end)
...
task.defer(function()
    while RunningEnvirontments do
        if Configuration.Egg.AutoPlaceEgg and not Configuration.Waiting then
            PlaceEggsUntilFull(Configuration.Egg.PlaceArea or "Any")
        end
        task.wait(tonumber(Configuration.Egg.AutoPlaceEgg_Delay) or 1.0)
    end
end)
-- ===== /Auto Place Egg =====
...
task.defer(function()
    while RunningEnvirontments do
        if Configuration.Pet.AutoPlacePet and not Configuration.Waiting then
            PlacePetsUntilFull(Configuration.Pet.PlaceArea or "Any")
        end
        task.wait(tonumber(Configuration.Pet.AutoPlacePet_Delay) or 1.0)
    end
end)
-- ===== /Auto Place Pet =====
