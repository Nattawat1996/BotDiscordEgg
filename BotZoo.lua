if game.PlaceId == 105555311806207 then
    ...
    table.insert(EnvirontmentConnections,Pet_Folder.ChildAdded:Connect(function(pet)
        ...
        if pet and pet:GetAttribute("UserId") == PlayerUserID then
            ...
            },{__index = (function(tb,ind) if ind == "Coin" then return (CashTXT and GetCash(CashTXT.Text)) end return rawget(tb,ind) end)})
        end
    end))
    ...
    local Configuration = {
        ...
        Players = {
            SelectPlayer = "",
            SelectType = "",
            -- ▼▼ เพิ่มใหม่สำหรับ Gift Pet Filter ▼▼
            GiftPet_FilterType = "All",        -- "All" | "Match Pet" | "Match Mutation" | "Match Pet&Mutation"
            GiftPet_Pets = {},                 -- รายการ Type ที่ต้องการส่ง
            GiftPet_Mutations = {},            -- รายการ Mutation ที่ต้องการส่ง
        },
        Event = {
            AutoClaim = false,
            AutoClaim_Delay = 3,
                    ...
                    Buttons = {
                        {
                            ...
                            Callback = function()
                                ...
                                if GiftType == "Pets" then
                                    -- ▼▼ ปรับลอจิกส่ง Pet: รองรับ Filter Type/Mutation ▼▼
                                    ...
                                    for _,PetDataNode in pairs(OwnedPetData:GetChildren()) do
                                        if PetDataNode and not PetDataNode:GetAttribute("D") then
                                            local uid = PetDataNode.Name
                                            local meta = OwnedPets[uid] -- มีเมื่อ pet spawn บนแมพ
                                            ...
                                            local function passFilter()
                                                ...
                                                elseif filterType == "Match Pet&Mutation" then
                                                    return (pType ~= nil) and (pMuta ~= nil)
                                                        and (wantPets[pType] == true)
                                                        and (wantMutas[pMuta] == true)
                                                end
                                                return false
                                            end

                                            -- หากไม่มี meta (ไม่รู้ชนิด/มิวเทชัน) อนุญาตเฉพาะกรณี All
