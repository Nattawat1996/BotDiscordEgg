...
local placeEggDropdown = Tabs.EggTab:Dropdown({
...
local placeMutationDropdown = Tabs.EggTab:Dropdown({
...
local autoPlaceToggle = Tabs.EggTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state and not autoPlaceThread then
            ...
            WindUI:Notify({ Title = "🏠 Auto Place Egg", Content = "Started - Placing pets automatically! 🎉", Duration = 3 })
        elseif (not state) and autoPlaceThread then
            cleanupPlaceConnections()
            WindUI:Notify({ Title = "🏠 Auto Place Egg", Content = "Stopped", Duration = 3 })
        end
    end
})
