...
local function updateAvailablePets()
    local pets = listAvailableEggUIDs()
    availablePets = {}
...
local function registerUIElements()
    ...
    registerIfExists("autoPlaceEnabled", autoPlacePetToggle)
...
do
    ...
    uiTab:Toggle({
        ...
        Callback = function(state)
            ...
            if state and not placePetsThread then
                placePetsThread = task.spawn(function()
                    runAutoPlacePets()
                    placePetsThread = nil
                end)
                if WindUI and WindUI.Notify then
                    WindUI:Notify({ Title="Place Pets", Content="เริ่มทำงานแล้ว ✨", Duration=3 })
                end
