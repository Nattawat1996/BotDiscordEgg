...
local _partPrev = setmetatable({}, { __mode="k" })
local _particlePrev = setmetatable({}, { __mode="k" })
local _togglePrev   = setmetatable({}, { __mode="k" })
...
function runAutoCollect(ctx)
    ...
    while not ctx.cancelled and RunningEnvirontments do
        ...
        ctx.sleep(tonumber(Configuration.Main.Collect_Delay) or 30)
    end
end

-- Auto Feed Pet (เฉพาะ Big)
function runAutoFeed(ctx)
    ...
    while not ctx.cancelled and RunningEnvirontments do
        ...
        ctx.sleep(tonumber(Configuration.Pet.AutoFeed_Delay) or 3)
    end
end

-- Auto Collect Pet (with Area + ALL support)
function runAutoCollectPet(ctx)
    local CharacterRELocal = GameRemoteEvents:WaitForChild("CharacterRE",30)
    local function passArea(uid)
    ...
    while not ctx.cancelled and RunningEnvirontments do
        ...
        ctx.sleep(tonumber(Configuration.Pet.CollectPet_Delay) or 5)
    end
end

-- Auto Hatch (with area filter)
function runAutoHatch(ctx)
    ...
    while not ctx.cancelled and RunningEnvirontments do
        ...
        ctx.sleep(tonumber(Configuration.Egg.Hatch_Delay) or 15)
    end
end

-- Auto Claim Event Quests
function runAutoClaim(ctx)
    local Tasks; local EventRE = ResEvent and GameRemoteEvents:WaitForChild(tostring(ResEvent).."RE",30)
    if EventTaskData then Tasks = EventTaskData:WaitForChild("Tasks",30) end
    while not ctx.cancelled and RunningEnvirontments do
        ...
        ctx.sleep(tonumber(Configuration.Event.AutoClaim_Delay) or 3)
    end
end

-- Auto Buy Egg
function runAutoBuyEgg(ctx)
    ...
    while not ctx.cancelled and RunningEnvirontments do
        ...
        ctx.sleep(tonumber(Configuration.Egg.AutoBuyEgg_Delay) or 1)
    end
end

-- Auto Place Egg
function runAutoPlaceEgg(ctx)
    local CharacterRELocal = GameRemoteEvents:WaitForChild("CharacterRE", 30)
    while not ctx.cancelled and RunningEnvirontments do
        ...
        ctx.sleep(tonumber(Configuration.Egg.AutoPlaceEgg_Delay) or 1.0)
    end
end
...
function runAutoPlacePet(ctx)
    ...
    while not ctx.cancelled and RunningEnvirontments do
        ...
        ctx.sleep(tonumber(Configuration.Pet.AutoPlacePet_Delay) or 1.0)
    end
end

-- Auto Buy Food
function runAutoBuyFood(ctx)
...
function runAutoLottery(ctx)
    ...
    while not ctx.cancelled and RunningEnvirontments do
        ...
        ctx.sleep(tonumber(Configuration.Event.AutoLottery_Delay) or 60)
    end
end
